...
- $𝛿: (Q\𝖠 x Σ) → (Q x Σ x {L, S, R})$ is a state-transition relation. The domain _(Q\𝖠 x Σ)_ defines all machines states (minus the final states) and symbols on tape, the codomain _(Q x Σ x {L, R, S})_ defines the new state of the machine, symbol writen to tape, and move of the head: Left, Right, or Stay,
